...
const rules = {
  email: [email => email && formRegx.email['pattern'].test(email) || formRegx.email['message']],
  username: [name => name && formRegx.username['pattern'].test(name) || formRegx.username['message']],
  password: [password => password && formRegx.password['pattern'].test(password) || formRegx.password['message'],
    password => password && (form.password === form.confirmPassword) || '비밀번호가 다릅니다.'],
  confirmPassword: [password => password && (form.password === form.confirmPassword) || '비밀번호가 다릅니다.'],
    ...
    <q-list class="q-gutter-md">
      ...
      <q-item>
        <q-item-section>
          ...
          <q-input
            ref="usernameRef"
            type="text" class="full-width" dense clearable
            v-model="form.username"
            :label="placeholder.username"
            lazy-rules :rules="rules.username">
            <template v-slot:append>
      ...
      <q-item>
        <q-item-section>
          <q-item-label class="q-pb-xs">Add Open AI Service Api Key</q-item-label>
          <q-input
            ref="accessTokenRef"
          ...
          >
            <template v-slot:append>
              <q-btn round dense flat icon="send" @click="alertAddOpenAIService"
                     type="submit" :disable="userStore.isOpenAIEnabled"
              />
            </template>
          </q-input>
        </q-item-section>
    ...
    <div class="col" style="margin-right: 50px; margin-left: 50px;">
      <div class="row q-gutter-sm ">
        <q-btn
          ...
          :disable="userStore.isGithubEnabled" color="transparent" unelevated
